...
Page({
  ...
  getData() {
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        this.setData({ killerTalents, survivorTalents });
      },
      fail: (err) => console.error('天赋加载失败', err)
    });
    ...
    wx.request({
      ...
      success: (res) => {
        ...
        this.setData({ killerRoles, survivorRoles });
      },
      fail: (err) => {
        wx.hideLoading();
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.statusCode === 200) {
          this.setData({ hunter: res.data });
        }
      },
      fail: (err) => console.log('最常用监管者请求失败', err),
    });
    ...
    wx.request({
      ...
      success: (res) => {
        if (res.statusCode === 200) {
          this.setData({ originRecords: res.data });
          ...
          const formatGameList = gameList.map(game => {
            const isHunter = game.hunter?.user?.user_id == userId;
            const isSurvivor =
              game.survivor1?.user?.user_id == userId ||
              game.survivor2?.user?.user_id == userId ||
              game.survivor3?.user?.user_id == userId ||
              game.survivor4?.user?.user_id == userId;
          ...
          // 计算胜率
          const hunterRate = hunterTotal === 0 ? 0 : Math.round((hunterWin / hunterTotal) * 10000) / 100;
          const survivorRate = survivorTotal === 0 ? 0 : Math.round((survivorWin / survivorTotal) * 10000) / 100;
      ...
      fail: (err) => console.log('历史对局请求失败', err),
    });
  },
  ...
  openEditModal(index) {
    ...
    const editDetails = survivorDetails.map(detail => {
      ...
      return { survivor, result, talents };
    });
  ...
  /**
   * 预处理战绩数据（分离角色名和天赋）—— 保留备用，但实际已由 convertBackendDataToFront 处理
   */
  ...
  onShowRoleTalents(e) {
    ...
    if (killerMatch) {
      roleName = killerMatch[1];
    } else if (survivorMatch) {
      roleName = survivorMatch[1];
    }
